...
namespace XInputHelper
{
	...
	public class XInputSceneController
	{
		...
		private void UpdateView()
		{
			...
			if ((LeftThumbStickX != 0) || (LeftThumbStickY != 0) || (RightThumbStickX != 0) || (RightThumbStickY != 0))
			{
				...
				if (LeftThumbStickX != 0)
				{
					...
					if (distance < 0 && distance > -0.75)
						distance = -0.75;
                    //newCamera = newCamera.MoveForward(distance);
                ...
                {
                    //Distance depends on the altitude, but must have minimum values (one way and the other) to not be almost nothing when altitude gets to 0)
                    var distanceY = (double)LeftThumbStickY * newCamera.Location.Z / 40;
                    if (distanceY > 0 && distanceY < 0.75)
                        distanceY = 0.75;
                    if (distanceY < 0 && distanceY > -0.75)
                        distanceY = -0.75;
                    //newCamera = newCamera.MoveForward(distance);
                    newCamera = newCamera.SetPitch(90).MoveForward(distanceY).SetPitch(newCamera.Pitch);
                }
				...
				newCamera = newCamera.MoveForward(distance2).RotateTo((double)RightThumbStickX, 
                    newCamera.Pitch > 2 ? -(double)RightThumbStickY : RightThumbStickY < 0 ? -(double)RightThumbStickY : 0,
                    0);
		...
		private static void OnControllerPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			if (newCtrl != null)
				newCtrl.SetSceneView(d as Esri.ArcGISRuntime.UI.Controls.SceneView);
		}
	}
}
